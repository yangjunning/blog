...
## 函数挂起（function pending）

当两次点击的间隔的时间大于 setTimeout 的时间，函数防抖就没有办法避免多次调用函数。当我们请求接口的时候，不管用户间隔多久点击，只要接口没有返回结果，就不能重复请求接口。为了解决这个问题，我引入了函数挂起的概念，就是借助loading状态来控制函数是否可以执行。
